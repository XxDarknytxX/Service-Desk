...
-- Add department FK to users (only if it doesn't exist)
...
SET @sql = IF(@constraint_exists = 0, 
  ...
  'SELECT "Constraint fk_users_department already exists" AS message'
);
...
CREATE TABLE IF NOT EXISTS approval_rules (
  ...
  description TEXT,
  is_active BOOLEAN DEFAULT TRUE,
  applies_to_priority_id INT UNSIGNED NULL,
  applies_to_type_id INT UNSIGNED NULL,
  ...
  after_approval_agent_id INT NULL DEFAULT NULL,
  after_approval_team_id INT NULL DEFAULT NULL,
  default_approvers JSON DEFAULT NULL,
  notes_template TEXT DEFAULT NULL,
  require_all_approvers TINYINT(1) DEFAULT 0,
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  created_by INT NOT NULL,
