...
// Corporate-ONLY staff: a corporate team that isn't a hierarchy team. Business
// teams and executives belong to both directories, like admins.
const IN_CORP_ONLY_TEAM_SQL = `EXISTS (SELECT 1 FROM team_members tmo JOIN teams tco ON tco.id = tmo.team_id WHERE tmo.user_id = u.id AND ${corporateOnlyTeamSql("tco")})`;
